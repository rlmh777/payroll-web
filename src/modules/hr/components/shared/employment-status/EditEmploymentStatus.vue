  ...
  <q-dialog v-model="isOpen" position="right" @hide="onClose">
    <q-card class="drawer-card">
      <q-card-section class="row items-center q-pb-none">
        <div class="text-h6">Edit Employment Status</div>
        <q-space />
...
watch(() => props.employmentStatus, (item) => {
...
async function onSubmit() {
  ...
  const updated = await store.updateEmploymentStatus(form.value.id, form.value.name);
  if (updated) {
    $q.notify({ color: 'positive', position: 'top', message: 'Employment status updated.' });
    emit('updated');
    onClose();
  } else if (store.error) {
