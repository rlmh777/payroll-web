  ...
  <q-dialog v-model="isOpen" position="right" @hide="onClose">
    <q-card class="drawer-card">
      <q-card-section class="row items-center q-pb-none">
        <div class="text-h6">Edit Employee Status</div>
        <q-space />
...
watch(() => props.employeeStatus, (item) => {
...
async function onSubmit() {
  ...
  const updated = await store.updateEmployeeStatus(form.value.id, form.value.name);
  if (updated) {
    $q.notify({ color: 'positive', position: 'top', message: 'Employee status updated.' });
    emit('updated');
    onClose();
  } else if (store.error) {
