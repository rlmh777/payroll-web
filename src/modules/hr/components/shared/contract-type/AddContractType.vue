  ...
  <q-dialog v-model="isOpen" position="right" @hide="onClose">
    <q-card class="drawer-card">
      <q-card-section class="row items-center q-pb-none">
        <div class="text-h6">Add Contract Type</div>
        <q-space />
...
import { useContractTypeStore } from 'src/stores/contract-type-store';

const props = defineProps<{ modelValue: boolean }>();
const emit = defineEmits<{
  'update:modelValue': [value: boolean];
  saved: [contractTypeId: number];
}>();
...
async function onSubmit() {
  ...
  const created = await store.createContractType(form.value.name);
  if (created) {
    $q.notify({ color: 'positive', position: 'top', message: 'Contract type created.' });
