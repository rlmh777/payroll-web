<template>
  <q-page class="q-pa-md">
    <q-card flat>
      <SearchDegree v-model:modelValue="store.search" />
          ...
          <template v-slot:body-cell-actions="props">
            <q-td :props="props" class="text-right">
              <div class="action-buttons">
                ...
                >
                  <q-tooltip>Edit Degree</q-tooltip>
                </q-btn>
                <q-btn
                  flat
                  round
                ...
                >
                  <q-tooltip>Delete Degree</q-tooltip>
                </q-btn>
              </div>
            </q-td>
          </template>
    ...
    <UpdateDegree v-model="dialogOpen" />
  </q-page>
</template>
...
import { useDegreeStore, type Degree } from '../../stores/degree-store';
import SearchDegree from './SearchDegree.vue';
import UpdateDegree from './UpdateDegree.vue';
...
const degrees = computed(() => store.degrees);
...
const onRequest = async (props: { pagination: { page: number; rowsPerPage: number } }) => {
  ...
  await store.fetchDegrees(page, rowsPerPage);
  pagination.value.rowsNumber = store.total;
};

const onEdit = (row: Degree) => store.setDegreeToEdit(row);

const onDelete = (row: Degree) => {
  $q.dialog({
    ...
    .onOk(() => {
      void (async () => {
        try {
          await store.deleteDegree(row.id);
