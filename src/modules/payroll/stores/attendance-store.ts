...
import type { ClockingImportPayloadRow } from '@hr/utils/clocking-import-staging';
...
export const useAttendanceStore = defineStore('attendance', {
  ...
  actions: {
    ...
    async updateTimesheetComment(id: string, payload: { comment: string | null }) {
      this.error = null;

      try {
        const response = await fetch(`${API_URL}/timesheets/${id}/comment`, {
          method: 'PATCH',
          headers: this.buildJsonHeaders(),
          body: JSON.stringify(payload),
        });
        ...
        if (!response.ok) {
          throw new Error(await this.parseError(response, 'Failed to update timesheet comment.'));
        }
        ...
        this.error = error instanceof Error ? error.message : 'Failed to update timesheet comment.';
        return null;
      }
    },
