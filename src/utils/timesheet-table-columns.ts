...
export const TIMESHEET_TABLE_COLUMNS: QTableColumn[] = [
  ...
  { name: 'unpaidHours', label: 'Unpaid', field: 'unpaidHours', align: 'right' },
...
const STORAGE_KEY = 'timesheet_visible_columns_v7';

export function normalizeVisibleColumnNames(names: string[]): string[] {
  const allowed = new Set(TIMESHEET_TABLE_COLUMNS.map((column) => column.name));
  const selected = new Set(
    names.filter((name) => allowed.has(name) && name !== TIMESHEET_ACTIONS_COLUMN),
  );
