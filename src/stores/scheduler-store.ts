...
import { normalizeCalendarEmployees } from 'src/utils/calendar-employment-utils';
import { canViewAllSchedulerEmployees, isSchedulerSupervisor } from 'src/utils/scheduler-access';
import type { SchedulerSortBy, SchedulerViewBy } from 'src/utils/scheduler-utils';
...
const SCHEDULER_ROW_HEIGHT_PX = 72;
const SCHEDULER_CHROME_HEIGHT_PX = 200;
const MIN_EMPLOYEES_PER_PAGE = 25;
const MAX_EMPLOYEES_PER_PAGE = 100;

function estimateEmployeesPerPage(): number {
  if (typeof window === 'undefined') {
    return MIN_EMPLOYEES_PER_PAGE;
  }

  const availableHeight = window.innerHeight - SCHEDULER_CHROME_HEIGHT_PX;
  const rowsThatFit = Math.ceil(availableHeight / SCHEDULER_ROW_HEIGHT_PX) + 2;

  return Math.min(MAX_EMPLOYEES_PER_PAGE, Math.max(MIN_EMPLOYEES_PER_PAGE, rowsThatFit));
}
  ...
  actions: {
    ...
    async resolveEmployees(role: string, currentEmployee: CalendarEmployee | null) {
      ...
      if (canViewAllSchedulerEmployees(normalizedRole)) {
        this.employeesMode = 'paginated';
        this.employeesPerPage = estimateEmployeesPerPage();
