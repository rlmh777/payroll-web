...
import { useAttendanceStore, type TimesheetRow } from './attendance-store';
import { useSchedulerStore } from './scheduler-store';
import { getDateRangeForView } from 'src/utils/scheduler-utils';
import {
...
} from 'src/utils/timesheet-table-columns';
...
export const useTimesheetStore = defineStore('timesheetBrowse', {
  ...
  actions: {
    ...
    async fetchTimesheetRows(page = 1, rowsPerPage = 500) {
      const attendanceStore = useAttendanceStore();
      await attendanceStore.fetchTimesheets(this.activeFilters, page, rowsPerPage);
    },
  },
});
