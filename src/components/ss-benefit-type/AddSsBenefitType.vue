  ...
  <q-dialog v-model="isOpen" position="right" @hide="onClose">
    <q-card class="drawer-card">
      <q-card-section class="row items-center q-pb-none">
        <div class="text-h6">Add SS Benefit Type</div>
        <q-space />
...
import { useSsBenefitTypeStore } from 'src/stores/ss-benefit-type-store';

const props = defineProps<{ modelValue: boolean }>();
const emit = defineEmits<{
  'update:modelValue': [value: boolean];
  saved: [benefitTypeId: number];
}>();
...
async function onSubmit() {
  ...
  const created = await store.createBenefitType(form.value.name);
  if (created) {
    $q.notify({ color: 'positive', position: 'top', message: 'Benefit type created.' });
