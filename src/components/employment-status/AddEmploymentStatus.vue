  ...
  <q-dialog v-model="isOpen" position="right" @hide="onClose">
    <q-card class="drawer-card">
      <q-card-section class="row items-center q-pb-none">
        <div class="text-h6">Add Employment Status</div>
        <q-space />
...
import { useEmploymentStatusStore } from 'src/stores/employment-status-store';

const props = defineProps<{ modelValue: boolean }>();
const emit = defineEmits<{
  'update:modelValue': [value: boolean];
  saved: [employmentStatusId: number];
}>();
...
async function onSubmit() {
  ...
  const created = await store.createEmploymentStatus(form.value.name);
  if (created) {
    $q.notify({ color: 'positive', position: 'top', message: 'Employment status created.' });
